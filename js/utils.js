...
function ResetRequestFields() {
    ...
    dojo.byId("txtSelectedRequestLocation").value = "";
    ...
    dojo.byId('divRequestLocation').style.display = "none";
...
//Show toggle request locations
function ToggleRequestLocationList() {
    dojo.byId("divRequestLocation").style.width = (dojo.coords("divDropdown").w - 2) + "px";
    CreateScrollbar(dojo.byId("divCreateRequestContent"), dojo.byId("divCreateRequestScrollContent"));
    dojo.byId("divRequestLocation").style.display = (dojo.byId("divRequestLocation").style.display == "block") ? "none" : "block";
    dojo.byId("divCreateRequestContentscrollbar_handle").style.position = (dojo.byId("divRequestLocation").style.display == "block") ? "static" : "relative";
    CreateScrollbar(dojo.byId("divScrollBarContainer"), dojo.byId("divScrollBarContent"));
}
...
function BlurTextIsos() {
    ...
    dojo.byId("txtSelectedRequestLocation").blur();
...
function SubmitIssueDetails() {
    ...
    serviceRequestAttributes[serviceRequestFields.RequestLocationFieldName] = dojo.byId("txtSelectedRequestLocation").value;
...
function ValidateRequestData() {
    ...
    if (dojo.byId("txtSelectedRequestLocation").value.trim() == "") {
        ShowSpanErrorMessage("spanServiceErrorMessage", messages.getElementsByTagName("selectRequestType")[0].childNodes[0].nodeValue);
        return false;
    }
...
//Show add-comments view
function CreateRequestLocationList(serviceRequestLayerFields) {
    var serviceRequestFields;
    for (var i = 0; i < serviceRequestLayerFields.length; i++) {
        if (serviceRequestLayerFields[i].name == requestLocationLayerName) {
            serviceRequestFields = serviceRequestLayerFields[i].domain.codedValues;
            break;
        }
    }
    ...
    for (var i = 0; i < serviceRequestFields.length; i++) {
        ...
        td.innerHTML = serviceRequestFields[i].name;
        dojo.connect(td, "onclick", function (evt) {
            dojo.byId('txtSelectedRequestLocation').value = this.innerHTML;
            dojo.byId('divRequestLocation').style.display = "none";
            dojo.byId("divCreateRequestContentscrollbar_handle").style.position = "relative";
            if (evt.stopPropagation) evt.stopPropagation();
            return;
    ...
    container.appendChild(table);
    scrollbar_container.appendChild(container);
    dojo.byId('divRequestLocation').appendChild(scrollbar_container);
}
